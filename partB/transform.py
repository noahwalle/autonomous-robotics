...
def transform_pose(tf, pose):
    ...
    Applying a list of transformations to a single pose (e.g.,
    rotating a list of poses to plot them better):
    >>> transform_pose([(0, 0, 0), (1, 1, np.pi/4), (1, 1, np.pi/2)],
                        (1, 0, 0))
    array([[ 1.        ,  0.        ,  0.        ],
           [ 1.70710678,  1.70710678,  0.78539816],
           [ 1.        ,  2.        ,  1.57079633]])

    Applying a list of transformations to a list of poses (e.g.,
    ...
    Applying a transformation to a list of poses (e.g., adding
    predicted movement to particles):
    ...
    a_ = a.reshape(-1, 3)
    b_ = b.reshape(-1, 3)
    result = np.zeros(np.broadcast(tf, pose).shape,
                      dtype=np.double).reshape((-1, 3))

    # Do the actual calculation
    result[:, 0] = a_[:, 0] + b_[:, 0] * \
        np.cos(a_[:, 2]) - b_[:, 1] * np.sin(a_[:, 2])
    result[:, 1] = a_[:, 1] + b_[:, 0] * \
        np.sin(a_[:, 2]) + b_[:, 1] * np.cos(a_[:, 2])
    result[:, 2] = a_[:, 2] + b_[:, 2]
...
def inverse_transform(tf):
    ...
    # Do the actual calculation
    result[:, 0] = np.cos(tf_[:, 2]) * (-tf_[:, 0]) + \
        np.sin(tf_[:, 2]) * (-tf_[:, 1])
    result[:, 1] = -np.sin(tf_[:, 2]) * (-tf_[:, 0]) + \
        np.cos(tf_[:, 2]) * (-tf_[:, 1])
    result[:, 2] = -tf_[:, 2]
...
def find_transform(posea, poseb):
    ...
    The arguments can be array-likes of broadcastable shapes as in
    transform_pose.
